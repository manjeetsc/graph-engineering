...
that cycle well "loop engineering": deciding what the agent looks at before it starts, what
"done" actually means, and, the part that's easiest to skip and most costly to skip, who's
allowed to say no to its own work. A loop with no real check is just an
...
1. `graph-review` loads. It checks first whether this is even worth a graph: several
   ...
   reviewer shouldn't judge all at once, so yes, it qualifies. The word "thoroughly" is what
